...
for (i in 1:length (files)) {
 ...
 # y <-  las1@data$Y
  ...
  #y<-crowns@geometry$y
  #xy_coord <- data.frame(x, y)
  write.csv(crowns,paste0("S:\\Lassen_data\\Lassen_Lidar\\Doug_fir\\Co-ordinates\\Chmb_",i,".csv"))
...
polygon_files <- list.files("S:\\Lassen_data\\Lassen_Lidar\\AOI\\CHM_100", pattern = ".shp")
...
for (i in 1:length(polygon_files)) {
  ...
  raster1 <- raster(paste0("S:\\Lassen_data\\Lassen_Lidar\\AOI\\NDVI_100\\NDVI_", i, ".tif", sep= ""))
  polygon <- readOGR(paste0("S:\\Lassen_data\\Lassen_Lidar\\AOI\\CHM_100\\CHM_", i, ".shp", sep= ""))
  ...
  values <- c(raster::extract(raster1, site_spdf1, fun=mean))
  values1 <- c(raster::extract(raster1, site_spdf1, fun=sd))
  site_spdf1$NDVI<-values
  site_spdf1$NDVI_SD<-values1
  site_df <- as.data.frame(site_spdf1)
  write.csv(site_df, file= paste0("S:\\Lassen_data\\Lassen_Lidar\\AOI\\Extracted_NDVI100\\NDVI_",i, ".csv"))
}
...
raster<-raster("S:\\Lassen_data\\Lassen_Lidar\\AOI\\NDVI_100\\NDVI_1.tif")

plot(raster)
shape<- readOGR(dsn = "S:\\Lassen_data\\Lassen_Lidar\\AOI\\CHM_100", layer = "Chm_1")
plot(shape)
...
csv_files <- list.files(path = "D:\\Height_diameter\\Tree_data", pattern = "*.csv", full.names = TRUE)
...
write.csv(final_df, "D:\\Height_diameter\\Tree_data\\Merge.csv")

##############################################

# Set the folder path
folder_path <- "S:\\Lassen_data\\Lassen_Lidar\\AOI\\Extracted_con"

# Get a list of all the CSV files in the folder
file_list <- list.files(path = folder_path, pattern = "*.csv", full.names = TRUE)

# Define the function to add a column with the file number
add_file_num <- function(file) {
  # Read in the CSV file
  df <- read.csv(file)
  
  # Extract the file number from the file name
  file_num <- substring(basename(file),5)
  
  # Create a new column with the file number
  df$file_num <- file_num
  
  # Write the modified data frame back to the file
  write.csv(df, file, row.names = FALSE)
}

# Apply the function to each file in the list
lapply(file_list, add_file_num)
